...
public class ReviewSpeechWindow : Window
{
    ...
    public ReviewSpeechWindow(ReviewSpeechViewModel vm)
    {
        ...
        Title = vm.IsMultiSpeakerMode
            ? Se.Language.Video.TextToSpeech.ReviewAudioSegmentsMultiSpeakers
            : Se.Language.Video.TextToSpeech.ReviewAudioSegments;
        Width = 1100;
        Height = 700;
        MinWidth = 700;
        MinHeight = 690;
    ...
    {
        if (!vm.IsMultiSpeakerMode)
        {
            var classicGrid = TableViewExtras.MakeTableView(multiSelect: false);
            classicGrid.Margin = new Thickness(0, 10, 0, 0);
            classicGrid.Width = double.NaN;
            classicGrid.Height = double.NaN;
            classicGrid[!TableView.ItemsSourceProperty] = new Binding(nameof(vm.Lines));
            classicGrid[!TableView.SelectedItemProperty] = new Binding(nameof(vm.SelectedLine)) { Mode = BindingMode.TwoWay };

            classicGrid.Columns.Add(new SeTableViewColumn
            {
            ...
            classicGrid.DoubleTapped += (s, e) => vm.LineGridDoubleClicked();
            vm.LineGrid = classicGrid;

            var classicTextBox = new TextBox
            {
                AcceptsReturn = true,
            ...
            {
                classicTextBox.FontFamily = FontFamilyHelper.Make(Se.Settings.Appearance.SubtitleTextBoxAndGridFontName);
            }
            classicTextBox.WithAccessibleName(Se.Language.General.Text);

            var classicContainer = new Grid
            {
                RowDefinitions =
                {
                    new RowDefinition { Height = new GridLength(1, GridUnitType.Star) },
            ...
            classicContainer.Add(classicGrid, 0, 0);
            classicContainer.Add(classicTextBox, 1, 0);

            return new Border { Child = classicContainer };
        }
